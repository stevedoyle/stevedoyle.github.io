...
date: 2026-08-16
tags: [rdma, networking, ai-infrastructure, falcon, uet, mrc]
toc: true
---

RoCEv2 is the dominant RDMA transport for AI clusters today, but it's showing its age and limitations at scale.
The next generation of hyperscale AI networks is being built on three enhanced RDMA protocols: Falcon (Google), UET (Ultra Ethernet Consortium), and MRC (OCP), each targeting the same set of fundamental limitations that RoCEv2 can't solve.
...
Falcon, UET, and MRC each instantiate this differently.
...
UET is the most architecturally ambitious of the three: a clean-slate redesign rather than an extension of RC semantics.
...
These aren't really three competing bets so much as three points on a spectrum of how much you're willing to break from RC semantics:

| | Falcon | UET | MRC |
|---|---|---|---|
| Base model | New HW transport, RC-inspired | Ground-up redesign | Minimal RC extension |
| Ordering | Explicit ordered/unordered | Reordering-tolerant by design | RC-compatible, reordering via SACK bitmap |
| Loss + CC | SACK + NACK, delay-based CC | Trim + NACK, NSCC | SACK + NACK, NSCC |
| API surface | IB Verbs-compatible | New semantic layer | libibverbs-modeled, Write/Write-IMM only |
| Status | OCP, Google, Intel E2000 series | UEC spec | OCP, in production (OpenAI, Microsoft) |

The shared adoption of NSCC-style congestion control across UET and MRC, and Falcon's move from Google-internal to OCP, both suggest the industry is converging on primitives even where the transport framing differs.
